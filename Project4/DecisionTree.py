...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    while not currentNode.isleaf:
        currentNode = currentNode.children[classificationData[currentNode.attr]]
...
def remainder(examples,attrName,attrValues,className):
    ...
    #YOUR CODE HERE
    remainder = 0
    Attr = getAttributeCounts(examples,attrName,attrValues,className)
    total = len(examples)

    for a in Attr.values():
        remainder += float(sum(a.values())) / total * setEntropy(a.values())
    return remainder
...
def infoGain(examples,attrName,attrValues,className):
    ...
    """
    #YOUR CODE HERE
    c = getClassCounts(examples, className).values()
...
def giniGain(examples,attrName,attrValues,className):
    ...
    gain = 0
    for a in Attr.values():
        n = float(sum(a.values()))
        gain += n/total *giniIndex(a.values())
    if gain == 0:
        return sys.maxint
    else:
        return 1 / gain
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    classC = getClassCounts(examples, className)
    if setScoreFunc(classC.values()) == 0:
        return LeafNode(classC.keys()[0])
    if len(remainingAttributes) == 0:
        return LeafNode(defaultLabel)
    maxA = None
    maxGain = float("-inf")
    for attr in remainingAttributes:
        Attrvalue = attributeValues[attr]
        tempGain = gainFunc(examples,attr,Attrvalue,className)
        if tempGain > maxGain:
            maxGain = tempGain
            maxA = attr
    tree = Node(maxA)
    ...
    for item in remainingAttributes:
        if item != maxA:
            newRemain.append(item)
    mostCommon = getMostCommonClass(examples,className)
    for value in attributeValues[maxA]:
        Exs = getPertinentExamples(examples,maxA,value)
        subtreeNode = makeSubtrees(newRemain, Exs, attributeValues, className, mostCommon, setScoreFunc,gainFunc)
        tree.children[value] = subtreeNode
    return tree
