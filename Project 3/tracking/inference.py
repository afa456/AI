        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                if emissionModel[Truedist] > 0:
                    allPossible[p] = self.beliefs[p] * emissionModel[Truedist]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for part in self.particles:
                trueDistance = util.manhattanDistance(pacmanPosition, part)
                allPossible[part] += emissionModel[trueDistance]
            if allPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                for index in range(0, self.numParticles):
                    self.particles[index] = util.sample(allPossible)
    ...
    def elapseTime(self, gameState):
        ...
        newposs = util.Counter()
        for oldPos in allpossible.keys():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                newposs[newPos] = newposs[newPos] + prob * allpossible[oldPos]
        newParticles = []
        particles = self.particles
        newposs.normalize()

        for i in particles:
            newParticles.append(util.sample(newposs))
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            newParticles[particle] = newParticles[particle]+ 1
        return util.normalize(newParticles)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numGhosts):
            newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle)), i+1, self.ghostAgents[i])
