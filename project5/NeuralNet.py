...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        newlist = [1.0] + inActs
        w_sum = self.getWeightedSum(newlist)
        output = self.sigmoid(w_sum)
        return float(output)
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        newlist = [1.0] + inActs
        w_sum = self.getWeightedSum(newlist)
        output = self.sigmoidDeriv(w_sum)
        return float(output)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for x in range(len(newList)):
            newWeight = self.weights[x] + alpha * delta * newList[x]
            totalModification += abs(self.weights[x] - newWeight)
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                output.append(perceptron.sigmoidActivation(result[len(result)-1]))
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[len(allLayerOutput) - 2])
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    newdeltas = deltas[0]
                    delta = gPrime*sum([nextLayer[x].weights[neuronNum+1]*newdeltas[x] for x in range(len(nextLayer))])
            ...
            for numLayer in xrange(0,self.numLayers):
                ...
                delta_layer = deltas[numLayer]
                for numNeuron in xrange(len(layer)):
                    weightMod = layer[numNeuron].updateWeights(allLayerOutput[numLayer],alpha,delta_layer[numNeuron])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for input, Expout in examplesTest:
        output = nnet.feedForward(input)
        roundedoutput = [round(r) for r in output[- 1]]

        if roundedoutput == Expout:
            testCorrect += 1
        else:
            testError += 1
